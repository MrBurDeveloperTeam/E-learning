...
export function CreateCommunityPostDialog({ userId, communityId, communityName, draft, trigger }: { userId: string; communityId?: string; communityName?: string; draft?: CommunityPostDraft; trigger?: ReactElement }) {
  ...
  return <Dialog open={open} onOpenChange={next=>{if(!busy)setOpen(next)}}>
    ...
    <DialogContent showCloseButton={!busy} className="h-[min(860px,calc(100dvh-2rem))] overflow-hidden p-0 sm:max-w-3xl">
      <form noValidate onSubmit={submit} className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto]">
        <DialogHeader className="border-b bg-gradient-to-r from-primary/15 via-primary/5 to-transparent px-6 py-5 pr-14">
        ...
        <div className="min-h-0 overflow-y-auto overscroll-contain px-6 py-5">
          <div className="grid gap-6 md:grid-cols-[minmax(0,1.35fr)_minmax(240px,.85fr)]">
            <section className="space-y-5">
              ...
              <div className="space-y-2"><div className="flex items-center justify-between"><Label htmlFor="community-post-body">Post</Label><span className="text-[11px] text-muted-foreground">{body.length}/20,000</span></div><Textarea id="community-post-body" value={body} maxLength={20000} aria-invalid={Boolean(error)} className="min-h-64 resize-none text-base leading-7" onChange={event=>{setBody(event.target.value);setError(null)}} placeholder="Share your question, clinical insight, workflow, or useful resource…"/><p className="text-xs text-muted-foreground">Protect patient privacy and remove identifying information.</p></div>
            ...
            <aside className="space-y-5">
              ...
              <div><div className="mb-2 flex items-center gap-2"><ImagePlus className="size-4 text-primary"/><Label>Media</Label><span className="ml-auto text-[11px] text-muted-foreground">{files.length}/6</span></div><div role="button" tabIndex={createPost.isPending?-1:0} aria-label="Add images or videos" className={`flex min-h-40 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed p-5 text-center transition-all ${dragging?'scale-[1.01] border-primary bg-primary/10':'border-border bg-muted/20 hover:border-primary/50 hover:bg-primary/5'}`} onClick={()=>!createPost.isPending&&inputRef.current?.click()} onKeyDown={event=>{if((event.key==='Enter'||event.key===' ')&&!createPost.isPending)inputRef.current?.click()}} onDragOver={event=>{event.preventDefault();if(!createPost.isPending)setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={event=>{event.preventDefault();setDragging(false);if(!createPost.isPending)chooseFiles([...event.dataTransfer.files])}}><span className="grid size-11 place-items-center rounded-full bg-primary/10"><UploadCloud className="size-5 text-primary"/></span><span className="mt-3 text-sm font-semibold">Add photos or videos</span><span className="mt-1 text-xs leading-5 text-muted-foreground">Drop files or browse<br/>25 MB each · up to 6</span><input ref={inputRef} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" className="hidden" onChange={event=>{chooseFiles([...(event.target.files??[])]);event.target.value=''}} /></div></div>
        ...
        <DialogFooter className="mx-0 mb-0 rounded-none border-t bg-background/95 px-6 py-4 backdrop-blur sm:items-center"><span className="mr-auto hidden text-xs text-muted-foreground sm:block">Drafts sync across devices. Complete every section to publish.</span>{createPost.isPending?<Button type="button" variant="outline" onClick={()=>abortRef.current?.abort()}>Cancel upload</Button>:<DialogClose render={<Button type="button" variant="ghost" disabled={busy} />}>Cancel</DialogClose>}<Button type="submit" name="intent" value="draft" variant="outline" disabled={busy}>{saveDraft.isPending?<Loader2 className="animate-spin"/>:<Save/>}{saveDraft.isPending?'Saving…':'Save draft'}</Button><Button type="submit" name="intent" value="publish" disabled={busy} className="min-w-36">{createPost.isPending?<><Loader2 className="animate-spin"/>Publishing</>:<><Send/>Publish post</>}</Button></DialogFooter>
      </form>
    </DialogContent>
  </Dialog>
}
