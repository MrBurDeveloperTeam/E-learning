...
export function CommunityAdminPage() {
  ...
  const resolveReport = async (id: string, action: 'dismiss' | 'resolve' | 'hide') => {
    try { await reportMutation.mutateAsync({ id, action }); toast.success(action === 'hide' ? 'Content hidden and report resolved' : action === 'dismiss' ? 'Report dismissed' : 'Report resolved'); return true }
    catch (error) { toast.error(error instanceof Error ? error.message : 'Report could not be processed'); return false }
  }
  const resolveSelectedReports = async (action: 'dismiss' | 'hide') => {
    try {
      for (const id of selectedReports) {
        await reportMutation.mutateAsync({ id, action })
        setSelectedReports(current => { const next = new Set(current); next.delete(id); return next })
      }
      toast.success(`${selectedReports.size} report groups processed.`); setSelectedReports(new Set())
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Batch moderation failed'); throw error }
  }
  ...
  return <AdminLayout className="community-review-dashboard" title="Community review" subtitle="Review reported or automatically hidden content, manage reports, and review new communities.">
    ...
    <AdminTableShell title="Moderation queue" actionClassName="min-w-0 justify-end sm:flex-1" action={<AdminFilterTabs className="w-full max-w-full flex-nowrap overflow-x-auto [&>button]:flex-1 [&>button]:justify-center" value={tab} onChange={(value) => { setTab(value); setFilter(value==='audit'?'all':'pending') }} options={[{ value: 'reports', label: 'Reports', count: openReports }, { value: 'comments', label: 'Auto-hidden comments', count: pendingComments }, { value: 'communities', label: 'Communities', count: pendingGroups }, { value: 'verification', label: 'Verification', count: pendingVerification },{value:'audit',label:'Audit log'}]} />}>
      ...
      <div className="flex flex-col gap-3 border-b border-border px-5 py-3 sm:flex-row sm:items-center">
        ...
        {tab === 'reports' && selectedReports.size > 0 && <div className="flex gap-2"><Button size="sm" variant="outline" disabled={reportMutation.isPending} onClick={() => void resolveSelectedReports('dismiss').catch(()=>undefined)}>Dismiss {selectedReports.size}</Button><Button size="sm" variant="destructive" disabled={reportMutation.isPending} onClick={() => setDangerAction({title:'Hide reported content?',description:`This will hide content for ${selectedReports.size} selected report groups and resolve those reports.`,label:`Hide ${selectedReports.size} items`,run:()=>resolveSelectedReports('hide')})}>Hide {selectedReports.size}</Button></div>}
      </div>
