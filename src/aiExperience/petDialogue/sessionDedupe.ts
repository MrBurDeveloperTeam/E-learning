...
// E-Learning tabs on this origin. Written ONLY on an explicit user
...
// outlives the mount.
//
// NOTE: neither this file nor CatMascot.jsx's Close handler ever reads or
// writes `notifications.is_read` — Cat's local dismissal state and the
// database's read state remain two fully separate systems, unchanged by
// this fix. Only the existing CTA action (Home.tsx's
// handleElearningInsightAction) marks a notification read, exactly as
// before.
//
...
// different video id) is never suppressed by an unrelated dismissal, and
